    ...
    try:
        with mp_face_mesh.FaceMesh(
            ...
            while True:
                ...
                # Also exit if the window is closed via the window manager (close/X button)
                try:
                    if cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) < 1:
                        break
                except Exception:
                    # Older OpenCV builds or platforms may not support getWindowProperty; ignore errors
                    pass
    finally:
        cap.release()
        # Ensure OpenCV windows are destroyed when loop exits
        try:
            cv2.destroyAllWindows()
        except Exception:
            pass
        # Final blink count output
        print(json.dumps({"blink_count": blink_count}), flush=True)